...
function ResultBanner({ result }) {
  if (result === 'win') return <p className={styles.win}>Ganaste la partida</p>;
  if (result === 'lose') return <p className={styles.lose}>El agente gano la partida</p>;
  ...
  if (state.screen === 'config') {
    return (
      <section className={styles.page}>
        <h1>Configuracion de Partida</h1>
        ...
        <div className={styles.formCard}>
          ...
          <label>
            ...
            <select
              ...
              onChange={(event) =>
                setForm((prev) => ({ ...prev, totalRounds: Number(event.target.value) }))
              }
            >
          ...
          <tbody>
            {state.history.map((round) => (
              <tr key={round.round_num}>
                ...
                <td>{round.player_action === 'cooperate' ? '🤝' : '🗡'}</td>
                <td>{round.agent_action === 'cooperate' ? '🤝' : '🗡'}</td>
